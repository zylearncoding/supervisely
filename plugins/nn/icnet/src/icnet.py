...
# From https://github.com/hszhao/ICNet/blob/master/evaluation/eval_all.m
_pascal_means_rgb = np.array([123.68, 116.779, 103.939], dtype=np.float32)


def make_icnet_input(img_rgb):
    centered_rgb = img_rgb.astype(np.float32) - _pascal_means_rgb[np.newaxis, np.newaxis, :]
    # Pretrained Cityscapes weights are from Caffe, which uses BGR colors, so we adapt BGR as model input.
    # Make a copy since pytorch cannot make a tensor from an array view with negative strides.
    centered_bgr = centered_rgb[:, :, ::-1].copy()
    return torch.from_numpy(centered_bgr.transpose((2, 0, 1))).float()
...
class ICNet(nn.Module):
    ...
    def load_caffe_weights(self, model_path):
        """
