...
def refact_yolo_config(config, input_size, batch_size, subdivisions, num_classes, lr):
    config = config.replace('cust_height', str(input_size[0]))
    config = config.replace('cust_width', str(input_size[1]))
